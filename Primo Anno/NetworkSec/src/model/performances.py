...
def to_class_indices(y: np.ndarray) -> np.ndarray:
    """
    Converts a 2D one-hot encoded array into 1D class indices.
    If the array is already 1D, it is returned as-is.

    Input:
    - y: Array containing samples and their labels, either 1D or 2D one-hot encoded.

    Output:
    - y: 1D array of class indices.
...
def model_performances_multiclass(
        labels_names: list,
        y_true: np.ndarray,
        y_pred: np.ndarray,
        y_prob: np.ndarray,
    ...
    """
    Computes and prints all performance metrics for a multiclass classifier
    and saves a confusion matrix plot to disk.

    Inputs:
    - labels_names: List of class name strings used for display and reporting.
    - y_true: 1D array of true class indices.
    - y_pred: 1D array of predicted class indices.
    - y_prob: 2D array of predicted probabilities, used for AUC computation.
    - scenario: Evaluation phase label, either 'TRAINING' or 'TESTING'.
    - model_name: Name of the model, used for titles and file naming.
...
def model_performances_report_generation(
        accuracy: float,
        precision: float,
        recall: float,
        f1_macro: float,
