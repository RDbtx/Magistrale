...
def merge_and_filter_data(csv_files: list) -> pd.DataFrame:
    ...
    keeping only the columns defined in FEATURES.

    Input:
    - csv_files: List of file paths to the labeled CSV files to be processed.

    Output:
    - merged_data: Single merged DataFrame containing only the selected feature columns.
...
def save_merged_csv(data: pd.DataFrame, output_dir: str) -> str:
    ...
    Inputs:
    - data: Merged and filtered DataFrame to be saved.
    - output_dir: Directory where the output CSV will be written.

    Output:
    - output_path: Full file path where the CSV was saved.
...
def filtering(dataset_dir: str) -> str:
    """
    Discovers all labeled CSV files in the dataset directory, merges them into
    a single DataFrame keeping only the selected features, and saves the result
    to disk.

    Input:
    - dataset_dir: Root directory to search recursively for labeled CSV files.

    Output:
    - output_path: Path to the saved merged CSV file.
    ...
    csv_files = glob.glob(os.path.join(dataset_dir, "**", "*-l.csv"), recursive=True)
