...
def read_csv(path: str) -> pd.DataFrame:
    """
    Reads a CSV file from disk and returns a copy as a DataFrame.

    Input:
    - path: Path to the CSV file to read.

    Output:
    - df: DataFrame copy of the CSV contents.
...
def fuzzing_labeler(input_csv: str, out_csv: str, start_time: int, end_time: int) -> None:
    ...
    identified by null QUIC packet length, indicating non-QUIC packets sent to a QUIC server.
...
def http_stream_labeler(
    ...
    """
    Labels HTTP stream attack traffic across two separate time windows,
    identified by attack IP and specific HTTP/3 QPACK table capacity values.
...
def quic_enc_labeler(input_csv: str, out_csv: str, start_time: int, end_time: int) -> None:
    ...
    identified by non-null DNS transaction ID, indicating DNS-over-QUIC misuse.
...
def http_smuggler_labeler(input_csv: str, out_csv: str, start_time: int, end_time: int) -> None:
    ...
    identified by non-null URL-encoded form key.
...
def http_concurrent_labeler(
    ...
    The distinguishing column varies by server type: http.host for HTTP/1 servers,
    http2.length for HTTP/2 servers.
    ...
    - start_time: Start of the attack time window.
    - end_time: End of the attack time window.
    - cond_control: Column name used to identify attack packets.
...
def http_pause_labeler(
    ...
    Supports open-ended time windows by passing None as end_time.
    The distinguishing column varies by server type: http.host for HTTP/1 servers,
    http2.length for HTTP/2 servers.
    ...
    - out_csv: Path to save the labeled output CSV file.
    - start_time: Start of the attack time window.
    - end_time: End of the attack time window, or None for no upper bound.
    - cond_control: Column name used to identify attack packets.
...
def labeling(dataset_dir: str) -> None:
    """
    Runs the full labeling pipeline by applying each attack-specific labeler
    to all server captures across all attack categories.

    Input:
    - dataset_dir: Root directory containing all attack subdirectories with raw CSV files.
