...
class BootAnimator:
    ...
    Inputs:
    - asset_path: Path to the logo image asset.
    - page: Active Flet page instance.
    - on_complete: Callback invoked when the user clicks the enter button.
    """
    ...
    def handle_enter(self, _) -> None:
        """
        Invokes the on_complete callback when the enter button is clicked.
        """
        self.on_complete()

    def build_layout(self) -> ft.Container:
        """
        Assembles and returns the full boot screen layout as a Flet Container.

        Output:
        - layout: Root container holding all boot screen widgets.
        """
        return ft.Container(
            expand=True,
            bgcolor=BG,
    ...
    async def fade_in(self, *controls, steps: int = 11, delay: float = 0.05) -> None:
        """
        Gradually fades one or more controls from invisible to fully visible.

        Inputs:
        - controls: One or more Flet controls to fade in simultaneously.
        - steps: Number of opacity increments in the animation.
        - delay: Seconds to wait between each opacity step.
        """
    ...
    async def run_animation(self) -> None:
        """
        Runs the full boot animation sequence: fades in logo, title, progress bar,
        boot messages, and finally the enter button with a pulsing border effect.
        """
...
# =====================================
# ---     Boot Screen Entry Point   ---
# =====================================

def show_boot_screen(asset_path: str, page: ft.Page, on_complete) -> None:
    """
    Clears the page, mounts the boot screen layout, and starts the animation.
