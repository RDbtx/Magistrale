...
LOGO_PATH = "src/gui/assets/netwatch_logo.png"
...
def resource_path(relative_path: str) -> str:
    """
    Resolves the absolute path to resource.

    Inputs:
    - relative_path: Relative path to the resource file.

    Output:
    - absolute_path: Absolute path to the resource.
    """
    try:
...
def make_cp_button(label: str, bg: str, fg: str, hover_bg: str, on_click) -> ft.Button:
    """
    Creates a styled control panel button with hover and disabled states.

    Inputs:
    - label: Button label text.
    - bg: Background colour hex string for the default state.
    - fg: Foreground (text and border) colour hex string.
    - hover_bg: Background colour hex string for the hovered state.
    - on_click: Callback function invoked when the button is clicked.

    Output:
    - button: Configured ft.Button instance.
...
class AppController:
    ...
    Manages the firewall thread lifecycle, the GUI poll loop, and all panel wiring.
    ...
    def push_log(self, msg: str, level: str = "info") -> None:
        """
        Appends a message to the system log panel and flags a UI update.
    ...
    def update_counters(self, allowed: int, warnings: int, total: int) -> None:
        """
        Updates the top-bar stat counter labels.

        Inputs:
        - allowed: Current count of allowed packets.
        - warnings: Current count of warning packets.
        - total: Current total packet count.
    ...
    def set_online(self, on: bool) -> None:
        """
        Toggles the online/offline indicator and enables or disables control buttons.

        Inputs:
        - on: True to set the interface to online state, False for offline.
    ...
    def run_firewall(self, bus: GUIEventBus, iface: str) -> None:
        """
        Firewall thread target: instantiates and runs the GUIFirewall,
        posting a fatal error to the bus if an exception occurs.

        Inputs:
        - bus: Event bus instance for posting updates to the GUI.
        - iface: Network interface name to capture traffic from.
    ...
    def on_start(self, _) -> None:
        """
        Handles the INITIATE button click: resets all panels, resolves the
        selected interface, creates the event bus, and starts the firewall thread.
    ...
    def on_stop(self, _) -> None:
        """
        Handles the TERMINATE button click: signals the firewall to stop,
        clears all references, and sets the interface to offline.
    ...
    async def poll_loop(self) -> None:
        """
        Async loop that drains the event bus and updates the page every 150 ms.
        Runs entirely on the Flet event loop, so page.update() never races with
        lifecycle events fired from the OS.
        """
    ...
    def stop_poll(self, _) -> None:
        """
        Stops the poll loop on page disconnect.
        """
        self.poll_alive = False
    ...
    def build_top_strip(self) -> ft.Container:
        """
        Builds the decorative top title bar with the NETWATCH logo and timestamp.

        Output:
        - strip: Configured ft.Container for the top strip.
    ...
    def build_controls_strip(self) -> ft.Container:
        """
        Builds the controls bar containing the status indicator, stat cards,
        interface selector, and start/stop buttons.

        Output:
        - strip: Configured ft.Container for the controls strip.
    ...
    def build_footer(self) -> ft.Container:
        """
        Builds the bottom status bar with disclaimer and path text.

        Output:
        - footer: Configured ft.Container for the footer.
    ...
    def mount(self) -> None:
        """
        Assembles the full layout, adds it to the page, and starts the poll loop.
        """
        header = ft.Column(
...
def build_app(page: ft.Page) -> None:
    """
    Instantiates the AppController and mounts the main application layout.
...
def main(page: ft.Page) -> None:
    """
    Entry point: configures the page, then shows the boot screen before
    handing off to the main application view.

    Input:
    - page: Active Flet page instance provided by the Flet runtime.
